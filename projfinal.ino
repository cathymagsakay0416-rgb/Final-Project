...
// Gas settings
int gasBaseline = 0;       // baseline value after warm-up
int gasThreshold = 0;      // baseline + offset
#define GAS_OFFSET 15       // trigger threshold above baseline
#define GAS_CONSECUTIVE 4   // number of consecutive readings to confirm GAS
int gasCounter = 0;

// --- Function to read gas sensor and average readings ---
  ...
  if (currentMillis - lastReadTime >= readInterval) {
    ...
    if (flame == LOW) {
      ...
      if (gasCounter >= GAS_CONSECUTIVE) {
        ...
        currentState = SAFE; // wait for consecutive confirmation
      }
    } 
    else {
      currentState = SAFE;
    ...
    // Update display and outputs only if state changed OR always display SAFE
